...
export interface ChatMessageProps {
  ...
  isMe: boolean;
  isHostMsg: boolean;
  senderData?: RoomMember | null | undefined;
...
export const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  return (
    <div
      className={`flex flex-col w-full ${isMe ? 'items-end' : 'items-start'} ${isConsecutive ? 'mt-0.5' : 'mt-3'}`}
    >
      <div className={`flex items-end gap-2 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        {!isMe &&
          (!isConsecutive ? (
            <PartyAvatar
              ...
              alt={displayName}
              size={28}
              className="mb-1 border border-bd-filed-form-color"
            />
          ) : (
        ...
        <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
          ...
          {!isMe && !isConsecutive && (
            <div className="flex items-center gap-1.5 mb-1 px-1">
              ...
              <span
                className={
                  isHostMsg
                    ? 'rounded bg-amber-500/15 px-1.5 py-px text-[9px] font-bold uppercase tracking-wider text-amber-400'
                    : 'rounded bg-bg-block px-1.5 py-px text-[9px] font-bold uppercase tracking-wider text-secondary'
                }
              >
          ...
          {!isConsecutive && (
            <div
              className={`flex items-center gap-1.5 mt-1 px-1.5 opacity-60 text-[9px] text-secondary ${isMe ? 'justify-end text-right' : 'justify-start text-left'}`}
            >
