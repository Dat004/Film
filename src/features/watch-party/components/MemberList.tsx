...
export function MemberList({
  ...
  const displayMembers = React.useMemo(() => {
    return sortedMembers.slice(0, MAX_MEMBERS);
  }, [sortedMembers]);
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2 max-h-[140px] overflow-y-auto pr-1 pb-1">
      {displayMembers.map((m) => (
        <div
          key={m.uid}
          className="flex items-center gap-2 px-2 py-1.5 rounded-full bg-transparent border border-black/5 dark:border-white/5 hover:shadow-sm transition-all h-9"
        >
          <img
          ...
          />
          <span
            className={`text-[13px] max-w-[110px] truncate ${
              m.uid === user.uid
