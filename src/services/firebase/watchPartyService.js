...
// Initialize Room
export const createRoom = async (host, filmId, filmData, password = null) => {
...
// Join Room
export const joinRoom = async (roomId, user, password = null) => {
  const db = getDatabase();
  const roomRef = ref(db, `watch_party_rooms/${roomId}`);
...
export const updateVideoSync = async (roomId, isPlaying, currentTime, currentEpisode = 0) => {
  const db = getDatabase();
...
export const sendReaction = async (roomId, user, emoji) => {
  const db = getDatabase();
...
export const kickMember = async (roomId, targetUserId) => {
  const db = getDatabase();
...
export const leaveRoom = async (roomId, userId, isHost, members) => {
  const db = getDatabase();
  ...
  const remainingCount = members ? Object.values(members).filter(m => m.uid !== userId).length : 0;
  if (remainingCount === 0) {
